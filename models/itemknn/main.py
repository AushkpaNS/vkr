...
def preprocess(
    df: pl.LazyFrame, interaction: str, val_size: int, deep: int
    ...
    train, val, test = timesplit.flat_split_train_val_test(
        df, val_size=val_size, test_timestamp=Constants.TEST_TIMESTAMP, deep=deep
    )
...
def item_knn(
    ...
    train, val, _ = preprocess(df, interaction, val_size=Constants.VAL_SIZE, deep=deep)
    ...
    # train final model
    train, _, test = preprocess(df, interaction, val_size=0, deep=deep)
